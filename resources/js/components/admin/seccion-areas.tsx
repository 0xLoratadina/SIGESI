...
const PRIORIDAD_CONFIG: Record<number, { clase: string; etiqueta: string }> = {
    ...
    2: {
        clase: 'bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200',
        etiqueta: 'Alta',
    },
    3: {
    ...
    4: {
        clase: 'bg-sky-100 text-sky-800 dark:bg-sky-900 dark:text-sky-200',
        etiqueta: 'Baja',
    },
    5: {
        clase: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200',
        etiqueta: 'Muy baja',
    },
};
...
function BadgePrioridad({ nivel }: { nivel: number }) {
    ...
    return (
        <Badge
            className={`inline-flex w-20 justify-center border-transparent ${config.clase}`}
        >
            {config.etiqueta}
        </Badge>
    );
...
const SeccionAreas = forwardRef<SeccionAreasRef, Props>(function SeccionAreas(
    { areas, busqueda },
    ref,
) {
    ...
    const totalPaginas = Math.max(
        1,
        Math.ceil(areasFiltradas.length / elementosPorPagina),
    );
        ...
        <div className="flex flex-col md:min-h-0 md:flex-1">
            {areasFiltradas.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12 text-center text-muted-foreground">
                    <p>
                        {busqueda.trim()
                            ? 'No se encontraron areas que coincidan con la busqueda.'
                ...
                <>
                    ...
                    >
                        <Table className="min-w-[550px] table-fixed">
                            <TableHeader>
                                <TableRow>
                                    <TableHead className="w-[30%] pl-[25px]">
                                        Nombre
                                    </TableHead>
                            ...
                            <>
                                <div className="grid gap-4 py-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="grid gap-2">
                                            <Label htmlFor="nivel_prioridad">
                                                Nivel de prioridad *
                                            </Label>
                                            <Select
                                                name="nivel_prioridad"
                                                defaultValue={String(
                                                    editando?.nivel_prioridad ??
                                                        '3',
                                                )}
                                            >
                                                <SelectTrigger>
                                            ...
                                            <InputError
                                                message={errors.nivel_prioridad}
                                            />
                                        </div>
                                    </div>
                                    {editando && (
